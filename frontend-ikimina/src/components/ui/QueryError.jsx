...
const QueryError = ({ error, onRetry, title = 'Could not load this data' }) => {
  ...
  return (
    <div className='mb-4 rounded-md border-l-4 border-red-400 bg-danger-subtle p-4' role='alert'>
        ...
        <div className='flex-1'>
          ...
          {onRetry && (
            <button
              type='button'
              onClick={onRetry}
              className='mt-2 rounded-md border border-danger/40 bg-surface px-3 py-1.5 text-sm font-medium text-danger hover:bg-danger-subtle'
            >
              Try again
            </button>
          )}
