...
import { useGetAllSubscriptionsQuery, useManuallyActivateSubscriptionMutation, useGetPaymentHistoryQuery } from '../adminApi';
import { FaCheck, FaEye, FaHistory, FaCreditCard, FaExclamationTriangle, FaBan } from 'react-icons/fa';
...
const SubscriptionManagement = () => {
  ...
  const getStatusBadge = (subscription) => {
    switch (subscription.status) {
      ...
      default:
        return <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-gray-800">{subscription.status}</span>;
    }
  };
  ...
  const gracePeriodSubscriptions = subscriptions?.filter(s => s.status === 'GRACE_PERIOD').length || 0;
                ...
                return (
                  <tr key={subscription.id} className="hover:bg-gray-50">
                    <td className="px-4 py-4">
                      <div className="text-sm font-medium text-gray-900">{subscription.groupName}</div>
                    ...
                    <td className="px-4 py-4">
                      ...
                      {subscription.suspensionReason && (
                        <div className="text-xs text-gray-500 mt-1">{subscription.suspensionReason}</div>
                      )}
                    </td>
      ...
      {showActivateModal && selectedSubscription && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Manually Activate Subscription</h3>
            <p className="text-gray-600 mb-4">
              Activate subscription for "{selectedSubscription.groupName}" ({selectedSubscription.planName})
            </p>
            <textarea
