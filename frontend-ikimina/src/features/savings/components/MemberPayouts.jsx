...
import { FaDollarSign, FaUser, FaCheck, FaFileExcel, FaDownload } from 'react-icons/fa';
import Button from '../../../components/ui/Button';
import toast from 'react-hot-toast';
import * as XLSX from 'xlsx';

const MemberPayouts = ({ cycleId, cycleStatus }) => {
  ...
  const handleBulkMarkAsPaid = async () => {
    try {
      await Promise.all(
        selectedMembers.map(id => markAsPaid(id).unwrap())
      );
  ...
  if (!payouts || payouts.length === 0) {
    return (
      <div className="bg-white p-6 rounded-lg shadow text-center">
        <p className="text-gray-500">No payouts available for this cycle</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-medium text-gray-900">Member Payouts</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-900">
                  ${payouts.reduce((sum, p) => sum + (p.payoutAmount || 0), 0).toLocaleString()}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <div className="flex items-center">
              <FaDollarSign className="h-8 w-8 text-blue-600 mr-3" />
              <div>
                <p className="text-sm font-medium text-blue-800">Group Retains (Ingoboka)</p>
                <p className="text-2xl font-bold text-blue-900">
                  ${payouts.reduce((sum, p) => sum + (p.totalIngoboka || 0), 0).toLocaleString()}
                </p>
              </div>
            </div>
          </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {payouts.map((payout) => (
                <tr key={payout.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4">
                    <div className="text-sm font-medium text-gray-900">{payout.memberName}</div>
                    <div className="text-xs text-gray-500">{payout.memberNumber}</div>
                  </td>
                  <td className="px-4 py-4 text-sm text-gray-900">
                    ${payout.totalUbwizigame?.toLocaleString() || 0}
                  </td>
                  <td className="px-4 py-4 text-sm text-gray-900">
                    ${payout.totalIngoboka?.toLocaleString() || 0}
                  </td>
